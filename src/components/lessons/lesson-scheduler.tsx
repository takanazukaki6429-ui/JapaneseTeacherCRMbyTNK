...
export function LessonScheduler({ studentId, studentName, onScheduled }: Props) {
    ...
    if (googleLink) {
        return (
            <div className="bg-[#f8e8e7] border border-[#d9a7ae]/40 rounded-2xl p-5 space-y-3">
                <div className="flex items-center gap-2 text-[#9c4f5a] font-bold text-sm">
                    <Check size={18} />
                    {recurrence === 'none' ? '予約が完了しました' : `${occurrences}回分の予約が完了しました`}
                </div>
                <p className="text-xs text-[#534344]">Googleカレンダーにこの予定を追加しますか？</p>
                <a
                    href={googleLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 w-full py-2 bg-white text-[#9c4f5a] font-bold text-sm rounded-xl hover:bg-[#f8e8e7] transition-colors border border-[#d9a7ae]/30"
                >
                    <Calendar size={15} />
                    Googleカレンダーに追加
                ...
                <button
                    ...
                    className="text-xs text-[#9c4f5a] hover:text-[#534344] w-full text-center underline"
                >
                    続けて予約する
                </button>
            </div>
    ...
    return (
        <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
                <div>
                    <label className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">日付</label>
                    <input
                        type="date"
                        value={date}
                        onChange={e => setDate(e.target.value)}
                        className="w-full px-3 py-2.5 bg-[#f1ebe1] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#f8e8e7] transition-colors"
                    />
                </div>
                <div>
                    <label className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">開始時間</label>
                    <input
                        type="time"
                        value={time}
            ...
            <div>
                <label className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">繰り返し</label>
                <div className="flex gap-2">
                    <select
                        value={recurrence}
                        onChange={e => setRecurrence(e.target.value as 'none' | 'weekly' | 'biweekly')}
                        className="flex-1 px-3 py-2.5 bg-[#f1ebe1] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#f8e8e7] transition-colors"
                    >
                    ...
                    {recurrence !== 'none' && (
                        <div className="flex items-center gap-1.5 w-24">
                            <input
                                ...
                                className="w-full px-3 py-2.5 bg-[#f1ebe1] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#f8e8e7] transition-colors"
                            />
                            <span className="text-xs text-[#534344] whitespace-nowrap">回</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            <button
                ...
                className="w-full inline-flex items-center justify-center gap-2 py-2.5 bg-[#9c4f5a] text-white font-bold text-sm rounded-full hover:scale-[1.01] transition-transform shadow-[0_4px_20px_rgba(156,79,90,0.25)] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
            >

...
`;
    ...
    if (fetchingScheduled || isAutoFilling) {
        return (
            <div className="flex h-screen flex-col items-center justify-center gap-3">
                ...
                {isAutoFilling && <p className="text-sm text-[#534344]">授業データをAIが解析中…</p>}
            </div>
        );
    }
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
            {autoFilled && (
                <div className="flex items-center gap-2 px-4 py-3 bg-[#f8e8e7] border border-[#d9a7ae]/40 rounded-2xl text-sm text-[#9c4f5a] font-medium">
            ...
            <div className="flex items-center gap-3">
                ...
                <h1 className="text-2xl font-bold tracking-tight text-[#3b2e2a]">レッスン記録</h1>
            </div>

            <form onSubmit={handleSubmit} className="space-y-5">
                {/* 基本情報 */}
                <div className="bg-white p-6 rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] space-y-4">
                    <h2 className="text-sm font-bold text-[#3b2e2a] border-b border-[#f1ebe1] pb-2">基本情報</h2>
                    <div>
                        <label htmlFor="date" className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">
                            日時 <span className="text-[#ba1a1a]">*</span>
                        </label>
                        <input
                            type="datetime-local" id="date" name="date" required
                            value={formData.date} onChange={handleChange}
                            className="w-full px-4 py-2.5 bg-[#f1ebe1] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#f8e8e7] transition-colors"
                        />
                    </div>
                </div>
                ...
                <div className="flex justify-end">
                    <button
                        ...
                        className="inline-flex items-center gap-2 px-4 py-2 bg-[#9c4f5a] text-white text-sm font-bold rounded-full hover:scale-[1.02] transition-transform shadow-[0_4px_20px_rgba(156,79,90,0.25)] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                    >
                ...
                {/* 学習内容 */}
                <div className="bg-white p-6 rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] border border-[#d9a7ae]/30 space-y-4">
                    <div className="flex items-center justify-between border-b border-[#f1ebe1] pb-2">
                        <h2 className="text-sm font-bold text-[#9c4f5a]">学習内容（AI分析用）</h2>
                        <span className="text-[10px] font-bold bg-[#f8e8e7] text-[#9c4f5a] px-2 py-0.5 rounded-full uppercase tracking-wide">重要</span>
                    </div>
                    <div>
                        <label htmlFor="topics" className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">文法・トピック</label>
                        <input type="text" id="topics" name="topics" value={formData.topics} onChange={handleChange}
                            className="w-full px-4 py-2.5 bg-[#f1ebe1] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#f8e8e7] transition-colors"
                            placeholder="例: 〜てはいけません, 許可を求める表現" />
                        <p className="text-[11px] text-[#534344]/60 mt-1">カンマ区切りで入力するとAIが認識しやすくなります</p>
                    </div>
                    <div>
                        <label htmlFor="vocabulary" className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">語彙・表現</label>
                        <textarea id="vocabulary" name="vocabulary" rows={2} value={formData.vocabulary} onChange={handleChange}
                            className="w-full px-4 py-2.5 bg-[#f1ebe1] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#f8e8e7] transition-colors resize-none"
                            placeholder="例: 規則、守る、厳しい" />
                    </div>
                    <div>
                        <label htmlFor="mistakes" className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">つまずき・弱点</label>
                        <textarea id="mistakes" name="mistakes" rows={3} value={formData.mistakes} onChange={handleChange}
                            className="w-full px-4 py-2.5 bg-[#fff0f0] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#ffe4e4] transition-colors border border-[#f4b8b8]/50 resize-none placeholder:text-[#ba1a1a]/30"
                ...
                {/* 生徒の様子・ネクスト */}
                <div className="bg-white p-6 rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] space-y-4">
                    <h2 className="text-sm font-bold text-[#3b2e2a] border-b border-[#f1ebe1] pb-2">生徒の様子・ネクストアクション</h2>
                    <div>
                        <label className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-2">理解度</label>
                        <div className="flex gap-1">
                            {[1, 2, 3, 4, 5].map((star) => (
                                <button key={star} type="button" onClick={() => setRating(star)}
                                    className={`p-1 transition-colors ${star <= formData.understanding_level ? 'text-[#d9a7ae]' : 'text-[#f1ebe1]'}`}>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="homework" className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">宿題</label>
                            <textarea id="homework" name="homework" rows={3} value={formData.homework} onChange={handleChange}
                                className="w-full px-4 py-2.5 bg-[#f1ebe1] rounded-xl text-sm text-[#3b2e2a] outline-none focus:bg-[#f8e8e7] transition-colors resize-none"
                                placeholder="ドリルP20-22" />
                        </div>
                        <div>
                            <label htmlFor="next_goal" className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">次回の目標</label>
                    ...
                    <div>
                        <label htmlFor="memo" className="block text-xs font-bold text-[#534344] uppercase tracking-wider mb-1.5">その他メモ</label>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] border border-[#d9a7ae]/30 space-y-4">
                    <div className="flex items-center justify-between border-b border-[#f1ebe1] pb-2">
                        <h2 className="text-sm font-bold text-[#9c4f5a] flex items-center gap-2">
                            <Globe size={15} /> 多言語フィードバック生成
                        </h2>
                        <span className="text-[10px] font-bold bg-[#f8e8e7] text-[#9c4f5a] px-2 py-0.5 rounded-full">生徒に送る文章</span>
                    </div>
                    <div>
                        <p className="text-xs text-[#534344] mb-2">生徒の母国語を選択</p>
                                ...
                                <button key={lang.code} type="button"
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${
                                        selectedLang === lang.code
                                            ? 'bg-[#9c4f5a] text-white shadow-sm'
                    ...
                    {multilingualFeedback && (
                        <div className="relative bg-[#f8e8e7] border border-[#d9a7ae]/40 rounded-2xl p-4">
                            <p className="text-sm text-[#3b2e2a] whitespace-pre-wrap leading-relaxed pr-16">{multilingualFeedback}</p>
                            <button type="button" onClick={handleCopyFeedback}
                                className={`absolute top-3 right-3 inline-flex items-center gap-1 px-2 py-1 text-xs font-bold rounded-lg transition-all ${
                                    feedbackCopied ? 'bg-[#dcfce7] text-[#166534]' : 'bg-white text-[#534344] hover:bg-[#ece8f3] border border-[#d9a7ae]/30'
                                }`}>
                ...
                <div className="flex justify-end pt-2 pb-0">
                    <button type="submit" disabled={loading}
                        className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#9c4f5a] text-white font-bold rounded-full hover:scale-[1.02] transition-transform shadow-[0_4px_20px_rgba(156,79,90,0.25)] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
                    >
                        {loading ? (
                            <>

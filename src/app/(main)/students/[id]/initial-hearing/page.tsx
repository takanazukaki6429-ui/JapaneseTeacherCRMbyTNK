...
const RoadmapGenerator = dynamic(() => import('@/components/roadmap/RoadmapGenerator'), {
    loading: () => (
        <div className="flex justify-center p-12">
            <Loader2 className="animate-spin text-[#9c4f5a]" size={28} />
        </div>
    ),
    ssr: false,
});
...
{"title": "教材のタイトル", "content": "", "cards": [{"front": "表面テキスト", "back": "裏面テキスト"}]}`,
        ...
        try {
            const res = await fetch('/api/ai', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ prompt, type: 'hearing_assist' }),   // 体験レッスン：補助の相談（利用実態を画面ごとに数えるため）
            });
    ...
    return (
        ...
        <div className="min-h-screen bg-[#f7f3ec]">
            {/* ヘッダー */}
            <div className="bg-white border-b border-[#f1ebe1] sticky top-0 z-50">
                <div className="max-w-4xl mx-auto px-4 h-14 flex items-center gap-4">
                    <Link href={`/students/${studentId}`} className="p-2 text-[#534344] hover:text-[#3b2e2a] hover:bg-[#f1ebe1] rounded-full transition-colors">
                        <ArrowLeft size={18} />
                    </Link>
                    <div>
                        <h1 className="font-bold text-[#3b2e2a] flex items-center gap-2 text-sm">
                            <Sparkles size={15} className="text-[#9c4f5a]" />
                            体験レッスン → ロードマップ作成
                        </h1>
                        <p className="text-xs text-[#534344]">{student.name}さん</p>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
                ...
                {(phase === 'input') && (
                    <>
                        {/* ステップ説明 */}
                        <div className="bg-gradient-to-r from-[#f8e8e7] to-white border border-[#d9a7ae]/30 rounded-2xl p-5">
                            <p className="text-sm font-bold text-[#3b2e2a] mb-3">使い方（3ステップ）</p>
                            <div className="flex items-start gap-6 text-xs text-[#534344]">
                                <div className="flex items-center gap-2">
                                    <span className="w-5 h-5 rounded-full bg-[#9c4f5a] text-white flex items-center justify-center font-bold flex-shrink-0 text-[10px]">1</span>
                                    体験レッスンのメモを入力<br/>（テキスト・音声・録音）
                                </div>
                                <div className="text-[#d9a7ae]">→</div>
                                <div className="flex items-center gap-2">
                                    <span className="w-5 h-5 rounded-full bg-[#9c4f5a] text-white flex items-center justify-center font-bold flex-shrink-0 text-[10px]">2</span>
                                    AIがレベル・目的を自動判定
                                </div>
                                <div className="text-[#d9a7ae]">→</div>
                                <div className="flex items-center gap-2">
                                    <span className="w-5 h-5 rounded-full bg-[#9c4f5a] text-white flex items-center justify-center font-bold flex-shrink-0 text-[10px]">3</span>
                                    スライダー確認→ロードマップ生成
                                </div>
                            </div>
                        </div>
                        ...
                        {/* 入力エリア */}
                        <div className="bg-white rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] p-6 space-y-4">
                            <div className="flex items-center justify-between">
                                <label className="text-sm font-bold text-[#3b2e2a]">体験レッスンのメモ</label>
                                <div className="flex items-center gap-2">
                                    {/* 録音ボタン（Chrome限定） */}
                                    {isChrome && (
                                        <button
                                            ...
                                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                                                isRecording
                                                    ...
                                                    : 'bg-[#f1ebe1] text-[#534344] hover:bg-[#f8e8e7] hover:text-[#9c4f5a]'
                                            }`}
                                    ...
                                    <button
                                        ...
                                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${
                                            isListening
                                                ? 'bg-[#9c4f5a] text-white animate-pulse'
                                                : 'bg-[#f1ebe1] text-[#534344] hover:bg-[#f8e8e7] hover:text-[#9c4f5a]'
                                        }`}
                            ...
                            {/* ステータスバナー */}
                            {isTranscribing && (
                                <div className="flex items-center gap-2 px-3 py-2 bg-[#f8e8e7] rounded-xl text-xs text-[#9c4f5a] font-bold">
                                    <Loader2 size={12} className="animate-spin" />
                                    録音を文字起こし中…
                                </div>
                            )}
                            {isListening && interimText && (
                                <div className="px-3 py-2 bg-[#f7f3ec] border border-[#d9a7ae]/30 rounded-xl text-sm text-[#534344] italic">
                                    {interimText}▋
                                </div>
                            )}
                            ...
                            <textarea
                                ...
                                rows={10}
                                className="w-full px-4 py-3 border border-[#dccfc4] rounded-xl text-sm text-[#3b2e2a] focus:outline-none focus:ring-2 focus:ring-[#9c4f5a] focus:border-transparent resize-y"
                                disabled={isTranscribing}
                            />

                            <div className="flex items-center justify-between">
                                <span className="text-xs text-[#534344]/60">
                                    {conversationNotes.length}文字
                                    {isListening && <span className="ml-2 text-[#9c4f5a] font-bold">● 音声入力中</span>}
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#9c4f5a] text-white text-sm font-bold rounded-xl hover:opacity-90 transition-opacity shadow-[0_4px_15px_rgba(156,79,90,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    <Sparkles size={15} />
                                    AIで判定する
                                </button>
                ...
                {phase === 'analyzing' && (
                    <div className="flex flex-col items-center justify-center py-24 gap-4">
                        <div className="w-16 h-16 rounded-full bg-[#9c4f5a] flex items-center justify-center animate-pulse">
                            <Sparkles size={28} className="text-white" />
                        </div>
                        <p className="text-sm font-bold text-[#3b2e2a]">AIが分析中…</p>
                        <p className="text-xs text-[#534344]">レベル・目的・ロードマップを推定しています</p>
                    </div>
                )}
                ...
                {phase === 'result' && result && (
                    <>
                        {/* AI判定結果（コンパクト） */}
                        <div className="bg-white rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] border border-[#d9a7ae]/40 overflow-hidden">
                            <div className="bg-[#9c4f5a] px-5 py-3 flex items-center justify-between">
                            ...
                            <div className="p-5 space-y-4">
                                {/* 要約 */}
                                <p className="text-sm text-[#3b2e2a] leading-relaxed bg-[#f7f3ec] rounded-xl p-3">{result.summary}</p>
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                    {[
                                        ...
                                        <div key={i} className="border border-[#f1ebe1] rounded-xl p-3 text-center">
                                            <p className="text-[10px] text-[#534344] mb-1">{item.label}</p>
                                            <div className="text-sm font-bold text-[#3b2e2a]">{item.value}</div>
                                        </div>
                                    ))}
                                </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    <div>
                                        <p className="text-xs font-bold text-[#3b2e2a] mb-2 flex items-center gap-1">
                                            <CheckCircle2 size={12} className="text-green-600" /> 重点項目
                                        </p>
                                        <ul className="space-y-1">
                                            {result.focus_areas.slice(0, 4).map((a, i) => (
                                                <li key={i} className="text-xs text-[#534344] flex items-start gap-1.5">
                        ...
                        {/* ロードマップ（AI自動セット、調整可） */}
                        <div className="bg-[#f8e8e7]/30 border border-[#d9a7ae]/30 rounded-2xl p-4">
                            <div className="flex items-center gap-2 mb-4">
                                <Sparkles size={15} className="text-[#9c4f5a]" />
                                <p className="text-sm font-bold text-[#9c4f5a]">
                            ...
                            />
                            {saving && (
                                <div className="flex items-center justify-center gap-2 py-4 text-[#9c4f5a]">
        ...
        {showMaterialModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
                    {/* ヘッダー */}
                    <div className="bg-[#9c4f5a] px-6 py-4 flex items-center justify-between">
                        <div>
                        ...
                        {!generatedMaterial ? (
                            <>
                                        ...
                                        <button
                                            ...
                                            className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 text-left transition-all ${
                                                selectedMaterialType === type
                                                    ? 'border-[#9c4f5a] bg-[#f8e8e7]/50'
                                                    : 'border-[#f1ebe1] hover:border-[#d9a7ae]'
                                            }`}
                                        >
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${
                                                selectedMaterialType === type ? 'bg-[#9c4f5a] text-white' : 'bg-[#f1ebe1] text-[#534344]'
                                            }`}>
                                                {info.icon}
                                            </div>
                                            <div>
                                                <p className="text-sm font-bold text-[#3b2e2a]">{info.label}</p>
                                                <p className="text-xs text-[#534344]">{info.desc}</p>
                                            </div>
                                        </button>
                                    ))}
                                </div>
                                ...
                                <div className="flex gap-3">
                                    <button
                                        onClick={handleModalClose}
                                        className="flex-1 py-3 rounded-xl border border-[#dccfc4] text-sm font-bold text-[#534344] hover:bg-[#f1ebe1] transition-colors"
                                    >
                                        あとで
                                    </button>
                                    <button
                                        ...
                                        className="flex-2 min-w-0 flex-1 py-3 rounded-xl bg-[#9c4f5a] text-white text-sm font-bold hover:opacity-90 transition-opacity disabled:opacity-60 flex items-center justify-center gap-2"
                                    >
                                        {generatingMaterial ? (
                            ...
                            <div className="space-y-4">
                                <div className="bg-[#f7f3ec] rounded-xl p-4 border border-[#f1ebe1]">
                                    <div className="flex items-center gap-2 mb-2">
                                        <CheckCircle2 size={15} className="text-green-600" />
                                        <p className="text-sm font-bold text-[#3b2e2a]">{generatedMaterial.title}</p>
                                    </div>
                                    {Array.isArray(generatedMaterial.cards) && generatedMaterial.cards.length > 0 && generatedMaterial.cards[0]?.front !== undefined ? (
                                        <div className="space-y-1.5 max-h-48 overflow-y-auto">
                                            {generatedMaterial.cards.slice(0, 5).map((card, i) => (
                                                <div key={i} className="text-xs bg-white border border-[#f1ebe1] rounded-lg p-2.5">
                                                    <span className="font-bold text-[#9c4f5a]">{card.front}</span>
                                                    <span className="text-[#534344] ml-2">→ {card.back}</span>
                                                </div>
                                            ))}
                                            {generatedMaterial.cards.length > 5 && (
                                                <p className="text-xs text-[#534344]/60 text-center">他 {generatedMaterial.cards.length - 5} 枚</p>
                                            )}
                                        </div>
                                    ) : (
                                        <p className="text-xs text-[#534344] whitespace-pre-wrap max-h-48 overflow-y-auto leading-relaxed">
                                ...
                                <button
                                    onClick={handleModalClose}
                                    className="w-full py-3 rounded-xl bg-[#9c4f5a] text-white text-sm font-bold hover:opacity-90 transition-opacity"
                                >
                                    生徒ページへ
                                </button>
                            </div>

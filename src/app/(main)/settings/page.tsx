...
export default function SettingsPage() {
    ...
    const SectionCard = ({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) => (
        <div className="bg-white rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] overflow-hidden">
            <div className="px-5 py-3.5 bg-[#f1ebe1] flex items-center gap-2.5">
                {icon}
                <h2 className="font-bold text-sm text-[#3b2e2a]">{title}</h2>
            </div>
    ...
    const RowItem = ({ label, desc, action }: { label: string; desc: string; action: React.ReactNode }) => (
        <div className="flex items-center justify-between py-3 border-b border-[#f1ebe1] last:border-0">
            <div>
                <p className="text-sm font-semibold text-[#3b2e2a]">{label}</p>
                <p className="text-xs text-[#534344] mt-0.5">{desc}</p>
            </div>
            {action}
        </div>
    );
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-5 pb-12">
            <div>
                <h1 className="text-2xl font-bold tracking-tight text-[#3b2e2a]">設定</h1>
                <p className="text-sm text-[#534344] mt-0.5">アプリケーションの各種設定を行います</p>
            </div>

            <SectionCard icon={<User size={16} className="text-[#9c4f5a]" />} title="アカウント設定">
                <MfaSection />
                <RowItem label="パスワード変更" desc="ログインパスワードの更新"
                    action={<button onClick={() => { setPwOpen(true); setPwError(''); setPwSuccess(false); }} className="text-xs bg-[#f1ebe1] text-[#534344] px-3 py-1.5 rounded-lg hover:bg-[#f8e8e7] hover:text-[#9c4f5a] transition-colors">変更</button>} />

                {pwOpen && (
                    <div className="mt-4 bg-[#f1ebe1] rounded-2xl p-5">
                        <form onSubmit={handlePasswordChange} className="space-y-3">
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full text-sm px-3 py-2.5 pr-10 rounded-xl border border-[#d9a7ae]/40 focus:outline-none focus:ring-2 focus:ring-[#d9a7ae] bg-white"
                                    required
                                />
                                <button type="button" onClick={() => setShowPw(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#534344]/60">
                            ...
                            <input
                                ...
                                className="w-full text-sm px-3 py-2.5 rounded-xl border border-[#d9a7ae]/40 focus:outline-none focus:ring-2 focus:ring-[#d9a7ae] bg-white"
                                required
                            />
                            ...
                            <div className="flex gap-2 pt-1">
                                <button type="button" onClick={() => setPwOpen(false)} className="text-xs px-4 py-2 rounded-xl bg-white text-[#534344] border border-[#d9a7ae]/30 hover:bg-[#f8e8e7] transition-colors">キャンセル</button>
                                <button type="submit" disabled={pwLoading || pwSuccess} className="text-xs px-4 py-2 rounded-xl bg-[#9c4f5a] text-white font-bold hover:scale-[1.02] transition-transform disabled:opacity-50 flex items-center gap-1.5">
            ...
            <SectionCard icon={<Database size={16} className="text-[#6b5b8c]" />} title="データ管理">
                <RowItem
                    label="データのエクスポート"
                    ...
                    action={
                        <button
                            ...
                            disabled={exporting}
                            className="inline-flex items-center gap-1.5 text-xs bg-[#f1ebe1] text-[#534344] px-3 py-1.5 rounded-lg hover:bg-[#f8e8e7] hover:text-[#9c4f5a] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                        >
            ...
            <SectionCard icon={<CreditCard size={16} className="text-[#9c4f5a]" />} title="プランとお支払い">
                <RowItem
                    label="プランとお支払い"
                    desc="ご利用中のプランの確認・お支払い方法の変更"
                    action={<Link href="/settings/billing" className="text-xs bg-[#f1ebe1] text-[#534344] px-3 py-1.5 rounded-lg hover:bg-[#f8e8e7] hover:text-[#9c4f5a] transition-colors">開く</Link>}
                />
            </SectionCard>
            ...
            <SectionCard icon={<AlertTriangle size={16} className="text-[#ba1a1a]" />} title="危険な操作">
                ...
                {deleteOpen && (
                    <div className="mt-4 bg-[#fff0f0] border border-[#f4b8b8] rounded-2xl p-5">
                        ...
                        <p className="text-xs text-[#534344] mb-2">
                        ...
                        <div className="flex gap-2 pt-3">
                            <button
                                ...
                                className="text-xs px-4 py-2 rounded-xl bg-white text-[#534344] border border-[#d9a7ae]/30 hover:bg-[#f8e8e7] transition-colors"
                            >
                                キャンセル
                            </button>
                            <button

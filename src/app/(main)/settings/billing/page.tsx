...
function BillingContent() {
    ...
    return (
        <div className="max-w-lg mx-auto space-y-6">
            <h1 className="text-2xl font-bold text-[#3b2e2a]">プラン・お支払い</h1>
            ...
            <div className="bg-white rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] p-6 space-y-5">
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-xs font-bold text-[#534344] uppercase tracking-wider mb-1">現在のプラン</p>
                        <p className="text-lg font-bold text-[#3b2e2a]">
                            {info?.is_free ? '無償プラン（招待）' : `プロプラン ${PLAN_PRICE_LABEL}/月`}
                        </p>
                    </div>
                    <CreditCard size={24} className="text-[#9c4f5a]" />
                </div>

                <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-[#534344]">ステータス</span>
            ...
            {info?.stripe_customer_id && !info?.is_free && (
                <div className="bg-white rounded-2xl shadow-[0_0_40px_rgba(156,79,90,0.06)] p-6">
                    <p className="text-sm font-bold text-[#3b2e2a] mb-1">支払い情報の管理</p>
                    <p className="text-xs text-[#534344] mb-4">
                        カードの変更・サブスクリプションのキャンセルはStripeのポータルで行えます。
                    </p>
                    <button
                        onClick={handlePortal}
                        disabled={portalLoading}
                        className="inline-flex items-center gap-2 px-4 py-2 bg-[#f1ebe1] text-[#3b2e2a] text-sm font-bold rounded-xl hover:bg-[#f8e8e7] transition-colors disabled:opacity-60"
                    >
            ...
            {!isActive && !info?.is_free && (
                <a
                    href="/pricing"
                    className="block w-full text-center py-3.5 bg-[#9c4f5a] text-white font-bold rounded-2xl hover:scale-[1.02] transition-transform shadow-[0_4px_24px_rgba(156,79,90,0.25)]"
                >
                    プランに加入する
                </a>
            )}
...
export default function BillingPage() {
    return (
        <Suspense fallback={<div className="flex h-64 items-center justify-center"><Loader2 className="animate-spin text-[#9c4f5a]" size={28} /></div>}>
            <BillingContent />
        </Suspense>
    );
}

                ...
                <div className="text-center space-y-2 pt-2 relative">
                    ...
                    <div className="absolute right-0 top-2 z-20">
                        <button
                            ...
                            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-white/80 backdrop-blur border border-gray-200 shadow-sm text-sm hover:bg-white transition-all"
                        >
                            <span>{locales[locale]?.flag}</span>
                            <ChevronDown className="w-3.5 h-3.5 text-gray-500" />
                        </button>
                        {langMenuOpen && (
                            <div className="absolute right-0 mt-1 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden min-w-[140px]">
                                {(Object.entries(locales) as [Locale, { flag: string; name: string }][]).map(([key, val]) => (
                                    <button
                                        key={key}
                                        onClick={() => { setLocale(key); setLangMenuOpen(false); }}
                                        className={`w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-blue-50 transition-colors text-left ${locale === key ? 'bg-blue-50 font-semibold text-blue-600' : 'text-gray-700'}`}
                                    >
                                        <span>{val.flag}</span>
                                        <span>{val.name}</span>
                                    </button>
                    ...
                    <div className="flex items-center justify-center gap-2">
                        ...
                        <h1 className="text-2xl font-bold text-[#020817]">{t.title}</h1>
                    </div>
                    ...
                    <CardContent className="p-6 space-y-6">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <div>
                                    <label className="text-sm font-semibold text-[#4b5563]">{t.currentLevel}</label>
                                    <p className="text-xs text-[#9ca3af]">{getLevelDescription(currentLevel, t)}</p>
                                </div>
                            ...
                            <Slider value={[currentLevel]} min={0} max={100} step={1} onValueChange={(val) => setCurrentLevel(val[0])} />
                        ...
                        {/* 日本語を学ぶ目的 */}
                        <div className="space-y-3">
                            <label className="text-sm font-semibold text-[#4b5563]">{t.purpose}</label>
                                ...
                                {purposeKeys.map((pid) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            style={{
                                                borderColor: isSelected ? pIcon.color : '#e5e7eb',
                                                backgroundColor: isSelected ? `${pIcon.color}10` : 'white',
                                                boxShadow: isSelected ? `0 0 0 1px ${pIcon.color}` : 'none',
                                            }}
                                        >
                                            <div
                                                className="w-9 h-9 rounded-full flex items-center justify-center"
                                                style={{ backgroundColor: isSelected ? `${pIcon.color}20` : '#f3f4f6' }}
                                            >
                                                <Icon className="w-4.5 h-4.5" style={{ color: isSelected ? pIcon.color : '#9ca3af' }} />
                                            </div>
                                            <span
                                                className="text-xs font-medium leading-tight"
                                                style={{ color: isSelected ? pIcon.color : '#4b5563' }}
                                            >
                                                {pLabel}
                                            </span>
                                        </button>
                            ...
                            {selectedPurpose && purposeIcon && (
                                <div
                                    ...
                                    style={{ backgroundColor: `${purposeIcon.color}10`, color: purposeIcon.color }}
                                >
                                    <purposeIcon.icon className="w-4 h-4 flex-shrink-0" />
                                    <span className="font-medium">{purposeDesc}</span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <div>
                                    <label className="text-sm font-semibold text-[#4b5563]">{t.targetLevel}</label>
                                    <p className="text-xs text-[#9ca3af]">{getLevelDescription(targetLevel, t)}</p>
                                </div>
                                <div className="flex items-end gap-1">
                                    <Input
                                        type="number"
                            ...
                            <Slider value={[targetLevel]} min={0} max={100} step={1} onValueChange={(val) => setTargetLevel(val[0])} />
                            <div className="flex justify-between text-xs text-[#9ca3af]">
                                <span>0 ({t.beginner})</span>
                                <span>50 (N3)</span>
                                <span>100 ({t.native})</span>
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center">
                                <label className="text-sm font-semibold text-[#4b5563]">{t.learningPeriod}</label>
                                <span className="font-bold text-lg">{periodMonths}{t.monthUnit}</span>
                            </div>
                            <Slider value={[periodMonths]} min={1} max={12} step={1} onValueChange={(val) => setPeriodMonths(val[0])} />
                            <div className="flex justify-between text-xs text-[#9ca3af]">
                        ...
                        {!canGenerate && selectedPurpose === null && (
                            <p className="text-xs text-center text-[#ef4444]">{t.selectPurpose}</p>
                        )}
                    </CardContent>
                </Card>

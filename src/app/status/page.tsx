...
export default function StatusPage() {
    ...
    return (
        <div className="min-h-screen bg-[#f6f3fb] flex flex-col items-center py-16 px-4">
            <div className="w-full max-w-2xl">
                <div className="flex items-center justify-between mb-8">
                    <h1 className="text-2xl font-bold text-[#3a3350]">ASTA ステータス</h1>
                    <button
                        onClick={fetchHealth}
                        disabled={loading}
                        className="inline-flex items-center gap-1.5 text-xs text-[#6b5ca5] hover:text-[#3a3350] transition-colors disabled:opacity-50"
                    >
                ...
                >
                    ...
                    <div>
                        ...
                        {lastChecked && (
                            <p className="text-xs text-[#484550] mt-0.5">
                ...
                <div className="bg-white rounded-2xl shadow-[0_0_40px_rgba(107,92,165,0.06)] overflow-hidden">
                    {data?.checks.map((check, i) => {
                        ...
                        return (
                            <div
                                key={check.name}
                                className={`flex items-center justify-between px-5 py-4 ${i > 0 ? 'border-t border-[#f0ebf8]' : ''}`}
                            >
                                <div className="flex items-center gap-3">
                                    <cMeta.Icon size={18} style={{ color: cMeta.color }} />
                                    <span className="text-sm font-medium text-[#3a3350]">
                                        {SERVICE_LABELS[check.name] || check.name}
                                    </span>
                                </div>
                                <div className="flex items-center gap-3">
                                    {check.latencyMs != null && (
                                        <span className="text-xs text-[#484550] tabular-nums">{check.latencyMs}ms</span>
                                    )}
                    ...
                    {(!data || data.checks.length === 0) && !loading && (
                        <div className="px-5 py-8 text-center text-sm text-[#484550]">
                            ステータス情報を取得できませんでした
                        </div>
                    )}
                </div>

                <p className="text-xs text-[#484550]/60 text-center mt-6">
                    このページは1分ごとに自動更新されます
                </p>
            </div>
        </div>

...
export function parseAppError(error: unknown): AppError {
    ...
    if (errMsg.includes('無効な招待コード')) {
        return {
            title: '招待コードが見つかりません',
            message: '入力された招待コードが登録されていません。',
            action: '入力し直してみてください（大文字・小文字、ハイフン「-」の位置をご確認ください）。それでも入らない場合は、コンサルタントにコードの確認をお願いしてください。'
        };
    }

    if (errMsg.includes('既に使用されています')) {
        return {
            title: 'この招待コードは使用済みです',
            message: '招待コードは1人1回だけ使えます。このコードは既に別の方が使っています。',
            action: 'コンサルタントに「新しい招待コードをください」とご連絡ください。新しいコードが届いたら、この画面でそのまま登録できます。'
        };
    }
